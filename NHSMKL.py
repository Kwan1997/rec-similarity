...
class NHSMKL(mySymmetricAlgo):
    ...
    def fit(self, trainset):
        ...
        delta = 0.00000008
        bigD = trainset.rating_scale[1] - trainset.rating_scale[0] + 1
        ...
        for itemi, ratesi in self.trainset.ir.items():
            for itemj, ratesj in self.trainset.ir.items():
                ...
                for rate in sorted(tempEmpiricalDistributioni):
                    FinalDistri.append((delta + (tempEmpiricalDistributioni[rate] / totali)) / (1 + delta * bigD))
                    FinalDistrj.append((delta + (tempEmpiricalDistributionj[rate] / totalj)) / (1 + delta * bigD))
                itemSimilarity[itemi, itemj] = (1 + 0.5 * (entropy(FinalDistri, FinalDistrj) + entropy(FinalDistrj, FinalDistri))) ** -1
            ...
            for userj, ratesj in self.trainset.ur.items():
                ...
                for itemi, ratei in ratesi:
                    for itemj, ratej in ratesj:
                        ...
                        Singularity = 1 - (1 + math.exp(-math.fabs(0.5 * (ratei + ratej) - 0.5 * (self.itemmeans[itemi] + self.itemmeans[itemj])))) ** -1
                        PSS += Proximity * Significance * Singularity * itemSimilarity[itemi, itemj]
...
if __name__ == '__main__':
    ...
    # mae, rmse, p, rec, f, npp, nsp = CalMetric().cvcalculate(NHSMKL, 5)
    mae, rmse, p, rec, f, npp, nsp = CalMetric().nocvcalculate(NHSMKL)
    print('#'*100)
    ...
    # from cdsds import CalMetric
    # resultsDict['mae'], resultsDict['rmse'], resultsDict['pre'], resultsDict['rec'], resultsDict['f1'], nppnsp['npp'], nppnsp['nsp'] = CalMetric().Curvecvcalculate(NHSMKL, fold=5, neighbours=neighbours2)
    # print('#'*100)
    # print('This is NHSMKL model')
    # print('#'*100)
