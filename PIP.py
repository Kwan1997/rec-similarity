                    ...
                    for itemj, ratej in ratesj:
                        if itemi == itemj:
                            if (ratei >= self.medianvalue and ratej >= self.medianvalue) or (ratei <= self.medianvalue and ratej <= self.medianvalue):
                                Proximity = (2 * self.maxminesmin + 1 - math.fabs(ratei - ratej)) ** 2
                                Impact = (math.fabs(ratei - self.medianvalue) + 1) * (math.fabs(ratej - self.medianvalue) + 1)
                                Popularity = 1 + ((ratei + ratej) * 0.5 - self.itemmeans[itemi]) ** 2 if (ratei > self.itemmeans[itemi] and ratej > self.itemmeans[itemi]) or (ratei < self.itemmeans[itemi] and ratej < self.itemmeans[itemi]) else 1
                            else:
                                Proximity = (2 * self.maxminesmin + 1 - 2 * math.fabs(ratei - ratej)) ** 2
                                Impact = ((math.fabs(ratei - self.medianvalue) + 1) * (math.fabs(ratej - self.medianvalue) + 1)) ** (-1)
                                Popularity = 1 + ((ratei + ratej) * 0.5 - self.itemmeans[itemi]) ** 2 if (ratei > self.itemmeans[itemi] and ratej > self.itemmeans[itemi]) or (ratei < self.itemmeans[itemi] and ratej < self.itemmeans[itemi]) else 1
                            PIP += Proximity * Impact * Popularity
                mySimilarity[useri, userj] = PIP
                # print(PIP)
                pbar.update(1)
        pbar.close()
...
if __name__ == '__main__':
    ...
    # # mae, rmse, p, rec, f, npp, nsp = CalMetric().cvcalculate(PIP, 5)
    # mae, rmse, p, rec, f, npp, nsp = CalMetric().nocvcalculate(PIP)
    # print('#'*100)
    # print('This is PIP model')
    # print('#'*100)
    # print('mae = ' + str(mae))
    # print('rmse = ' + str(rmse))
    # print('pre = ' + str(p))
    ...
    from cdsds import CalMetric
    resultsDict['mae'], resultsDict['rmse'], resultsDict['pre'], resultsDict['rec'], resultsDict['f1'], nppnsp['npp'], nppnsp['nsp'] = CalMetric().Curvecvcalculate(PIP, fold=5, neighbours=neighbours2)
    print('#'*100)
    print('This is PIP model')
    print('#'*100)
    ...
    np.save('./pip.npy', resultsDict)
    np.save('./pip2.npy', nppnsp)
